...
# https://github.com/CarlosManuelRodr/Fisica-Python/blob/master/Termodinamica/sistema_abierto_sim.py
...
class disk:
    def __init__(self, X, Y, VX, VY, R, pColor):
        ...
        self.vx = VX
        self.vy = VY
        self.obj = simple_sphere(pos=vec(X, Y, 0), radius=R, color=pColor)
...
class system:
    ...
    def __init__(self, coordX, coordY, systemSize, systemColor=color.white, systemWalls=[], maxVel=1):
        ...
        self.vMax = maxVel
        self.obj = box(pos=vec(coordX + systemSize / 2, coordY + systemSize / 2, -1), axis=vec(1, 0, 0),
                       height=self.size, width=0.2, length=self.size, texture=textures.wood, color=systemColor)
        wallSize = 1
        if len(systemWalls) != 0:
            self.walls = systemWalls
        if self.walls[0] == 1:
            self.wallArray.append(
                box(pos=vec(coordX - wallSize / 2, coordY + systemSize / 2, -0.5), axis=vec(1, 0, 0), height=self.size,
                    width=1, length=1))
        if self.walls[1] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize, coordY + systemSize / 2, -0.5), axis=vec(1, 0, 0), height=self.size,
                    width=1, length=1))
        if self.walls[2] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize / 2, coordY + systemSize, -0.5), axis=vec(0, 1, 0), height=self.size,
                    width=1, length=1))
        if self.walls[3] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize / 2, coordY, -0.5), axis=vec(0, 1, 0), height=self.size, width=1,
                    length=1))
    ...
    def addParticles(self, n, partColor=color.blue):
        ...
        while len(self.disks) < n:
            if (not self.diskOverlap(tempX, tempY)) and self.insideBox(tempX, tempY):
                self.disks.append(
                    disk(tempX, tempY, uniform(-self.vMax, self.vMax), uniform(-self.vMax, self.vMax), self.sigma / 2,
                         partColor))
                index = len(self.disks) - 1
                self.disks[index].obj.pos = vector(self.disks[index].x + self.size / 2 + self.x,
                                                   self.disks[index].y + self.size / 2 + self.y, 0)
            tempX = uniform(-self.size / 2, self.size / 2)
            tempY = uniform(-self.size / 2, self.size / 2)
    ...
    def moveParticles(self):
        for i in range(0, self.N):
            ...
            self.disks[i].obj.pos = vector(self.disks[i].x + self.size / 2 + self.x,
                                           self.disks[i].y + self.size / 2 + self.y, 0)
...
class piston:
    ...
    def __init__(self, coordX, coordY, myHeight):
        ...
        self.x = coordX
        self.y = coordY
        self.obj1 = box(pos=vec(coordX + self.systemSize / 2, coordY + self.sHeight / 2, -0.5), axis=vec(1, 0, 0),
                        height=self.sHeight, width=0.2, length=self.systemSize, color=color.green)
        self.obj2 = cylinder(pos=vec(coordX + self.systemSize / 2, coordY + self.sHeight / 2, -1), axis=vec(1, 0, 0),
                             radius=0.7, length=7)
...
class pistonSystem:
    ...
    def __init__(self, coordX, coordY, systemSize, pistonWall, systemColor=color.white, systemWalls=[], maxVel=1,
                 maxPistonVel=1):
        self.x = coordX
        self.y = coordY
        self.size = systemSize
        ...
        self.obj = box(pos=vec(coordX + systemSize / 2, coordY + systemSize / 2, -1), axis=vec(1, 0, 0), height=self.size,
                       width=0.2, length=self.size, texture=textures.wood, color=systemColor)
        wallSize = 1
        if len(systemWalls) != 0:
            self.walls = systemWalls
        ...
        if self.walls[1] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize, coordY + systemSize / 2, -0.5), axis=vec(1, 0, 0), height=self.size, width=1,
                    length=1))
        if self.walls[2] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize / 2, coordY + systemSize, -0.5), axis=vec(0, 1, 0), height=self.size, width=1,
                    length=1))
        if self.walls[3] == 1:
            self.wallArray.append(
                box(pos=vec(coordX + systemSize / 2, coordY, -0.5), axis=vec(0, 1, 0), height=self.size, width=1, length=1))
    ...
    def addParticles(self, n, partColor=color.blue):
        ...
        while len(self.disks) < n:
            if (not self.diskOverlap(tempX, tempY)) and self.insideBox(tempX, tempY):
                self.disks.append(
                    disk(tempX, tempY, uniform(-self.vMax, self.vMax), uniform(-self.vMax, self.vMax), self.sigma / 4,
                         partColor))
                index = len(self.disks) - 1
                self.disks[index].obj.pos = vector(self.disks[index].x + self.size / 2 + self.x,
                                                   self.disks[index].y + self.size / 2 + self.y, 0)
...
def move_piston_and_wall():
    move_piston()
    remove_pistion_walls()
...
pistonButton = button(pos=(0, 30), height=15, width=30, text='Move piston', bind=move_piston)
wallsButton = button(pos=(0, 10), height=15, width=30, text='Remove wall', bind=remove_pistion_walls)
moveAllButton = button(pos=(0, -10), height=15, width=40, text='Insert particles', bind=move_piston_and_wall)

if plotEnergy:
    gd = graph(width=600, height=300, title='Energia', xtitle='t', ytitle='E')
