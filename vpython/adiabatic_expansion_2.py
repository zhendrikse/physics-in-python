...
p_a, v_a = np.zeros((N, 3)), np.zeros(
    (N, 3))  # particle position array and particle velocity array, N particles and 3 for x, y, z
for i in range(N):
    p_a[i] = [2 * L * random() - L, 2 * L * random() - L,
              2 * L * random() - L]  # particle is initially random positioned in container
...
while stage == 0:
    ...
    for ij in hitlist:  # i,j encoded as i*Natoms+j
        ...
        if sum((p_a[i] - p_a[j]) * (
                v_a[i] - v_a[j])) < 0:  # only handling collision if two atoms are approaching each other
    ...
    if runs % 1000 == 0:
        ...
        SurfaceArea = 2 * (
                    container.length * container.width + container.width * container.height + container.height * container.length)
...
for v in arange(0., 4201. + dv, dv):  # theoretical speed distribution
    theory_high_T.plot(pos=(v, (deltav / dv) * N * 4. * pi * ((m / (2. * pi * k * T)) ** 1.5) * exp(
        (-0.5 * m * v ** 2) / (k * T)) * (v ** 2) * dv))
observation2 = ghistogram(graph=vdist, bins=arange(0., 4200., deltav),
