...
from vpython import canvas, scene, arrow, color, vec, pi, floor, arange, cos, sin, rate, cylinder, box, label, checkbox, wtext, slider, text, get_library
...
animation = canvas(align="top", forward=vec(0.37, -0.55, -0.75), width=600, height=450, background=color.black, title=title, range=11.5)
...
class PlaneWave:
  def __init__(self, k =2 * pi / 5, omega =2 * pi, amplitude=3.):
    self._arrows = [arrow(pos=vec(x, 0, 0), axis=vec(0, amplitude, 0), color=color.red, shaftwidth=0.2) for x in arange(-10, 10, 0.3)]
