...
class SunEarthMoonSystem:
    ...
    def update(self, program_speed):
        ...
        self._rotate(program_speed)

    def _rotate(self, program_speed):
        # Calculate the amount by which the position of the Earth and Moon change each loop cycle
        self._earth_angle -= earth_orbit_rate * program_speed  # we subtract to make counterclockwise orbits seen from above
...
class EarthArrows:
    def __init__(self, radius_earth, scale=0.5, latitude=52, tilt=23):
        self._scale = scale
...
def on_radio_button(event):
    if event.text == 'white background':
        ...
        earth_arrows.show() if event.checked else earth_arrows.hide()

animation = canvas(title=title, height=300, width=1200, range=astronomical_unit / 4., fov=radians(10))
animation.append_to_caption("\nAdjust the speed of the simulation using the slider\n")
background_button = checkbox(bind=on_radio_button, text='white background', name='background', checked=False)
earth_arrows_button = checkbox(bind=on_radio_button, text='earth arrows', name='arrows', checked=True)
speed_slider = slider(bind=set_speed, value=simulation_speed, min=0.0, max=1./24.)
animation.append_to_caption("\n\n")

sun_earth_moon = SunEarthMoonSystem()
earth_arrows = EarthArrows(radius_earth=sun_earth_moon.earth_radius(), latitude=52) # UK latitude, -38 for melbourne
...
graph_height = 70
graph_width = 480
daylight_graph = graph(width=graph_width, height=graph_height * 4, title="Hours with daylight", xmax=365, ymin=5, ymax=17)
...
while sun_earth_moon.day_count() <= 365:  # stop after one year
    rate(100)
    sun_earth_moon.update(simulation_speed)
    ...
    if sun_energy_transfer == 0 and daylight != 0:
        daylight = daylight * simulation_speed
