...
animation = canvas(title=title, width="800", height="600", caption=caption, center=vec(.031, -0.58, -0.19),
                   forward=vec(-0.32, -0.7, -0.64), range=1)
...
class Chain:
    def __init__(self, amount_hanging_over_edge=0.2):
        ...
        while rt.x <= 0:
            self._balls += [
                sphere(pos=rt, radius=self.ball_radius(), m=self._mass / self._total_amount_balls, p=vector(0, 0, 0),
                       on=True)]
            rt += vector(self._inter_ball_chain_length, 0, 0)
    ...
    def _update_forces(self):
        ...
        self._balls[0].F = self._individual_spring_constant * (mag(position_right_ball)
                                                               - self._inter_ball_chain_length) * norm(
            position_right_ball) + self._balls[0].m * gravitational_force_vector
        self._balls[-1].F = self._individual_spring_constant * (mag(position_left_ball)
                                                                - self._inter_ball_chain_length) * norm(
            position_left_ball) + self._balls[-1].m * gravitational_force_vector
        ...
        for i in range(1, self._total_amount_balls - 1):
            ...
            force_to_left = self._individual_spring_constant * (
                        mag(position_left_ball) - self._inter_ball_chain_length) * norm(position_left_ball)
            force_to_right = self._individual_spring_constant * (
                        mag(position_right_ball) - self._inter_ball_chain_length) * norm(position_right_ball)
...
table_length = 1
table_height = 0.1
table = box(pos=vector(-table_length / 2 - .01, -table_height / 2 - .01, 0),
            size=vector(table_length, table_height, 3 * table_height), opacity=0.3)

corner = sphere(pos=vector(0, chain.length() / 10, 0), radius=chain.ball_radius() / 2, color=color.yellow)
le = sphere(pos=vector(-(chain.length() - amount_hanging_over_edge), chain.length() / 10, 0),
            radius=chain.ball_radius() / 2, color=color.magenta)
be = sphere(pos=vector(chain.length() / 10, -amount_hanging_over_edge, 0), radius=chain.ball_radius() / 2,
            color=color.cyan)
