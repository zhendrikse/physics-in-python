...
class Ball(Moveable):
    def __init__(self, mass=1.5, position=vector(0, 0, 0), velocity=vector(0, 0, 0), radius=0.1, colour=color.yellow, elasticity=1.0, make_trail=False, draw=True):
    ...
    def _vpython_ball(self, mass, position, velocity, radius, colour, elasticity, make_trail):
        return sphere(pos=position, radius=radius, color=colour, velocity=velocity, mass=mass, elasticity=elasticity, make_trail=make_trail)
