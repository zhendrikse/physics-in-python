...
Alabel = label(pos=vector(Ax0, 0, Ay0), text="{:4.2f}".format(Ar[-1]), color=color.cyan, yoffset=-60, opacity=0)

source_arrow = arrow(pos=As0.pos, axis=vector(0, curve_vectors_a[0].y, 0), color=color.cyan)
target_arrow = arrow(pos=As1.pos, axis=vector(0, curve_vectors_a[-1].y, 0), color=color.cyan)
...
Bsource = arrow(pos=Bs0.pos, axis=vector(0, curve_vectors_b[0].y, 0), color=color.green)
Btarget = arrow(pos=Bs1.pos, axis=vector(0, curve_vectors_b[0].y, 0), color=color.green)

Xtarget = arrow(pos=Bs1.pos, axis=vector(0, curve_vectors_a[-1].y + curve_vectors_b[-1].y, 0), color=color.magenta)
...
while 1:
    ...
    for i in range(len(Ar)):
        vectors_a += [vector(0, amplitude_a * sin(wavenumber_k_a * Ar[i] - omega_a * t + phase_a), 0)]
        Ac.modify(i, y=amplitude_a * sin(wavenumber_k_a * Ar[i] - omega_a * t + phase_a))
    ...
    vectors_b = []
    for i in range(len(Ar)):
        vectors_b += [vector(0, amplitude_b * sin(wave_number_k_b * Ar[i] - omega_b * t + phase_b), 0)]
        Bc.modify(i, y=amplitude_a * sin(wavenumber_k_a * Ar[i] - omega_a * t + phase_a))
