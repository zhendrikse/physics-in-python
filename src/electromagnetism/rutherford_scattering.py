...
from src.toolbox.particle import Particle
...
xMax = 1e-13
animation = canvas(x=0, y=0, height=800, width=900, background=color.black, range=xMax * 2.5, title=title)
...
gold = Particle(position=vec(0, 0, 0), radius=6e-15, colour=color.yellow, mass=197e-3 / 6.02e23, charge=79 * 1.6e-19, velocity=vector(0, 0, 0))

trails = []
for nalpha in range(1000):
    rate(5)
    alpha_particle = Particle(position=source.beam(), radius=4e-15, colour=vector(.2, 1., 1.), mass=alpha_mass, charge=2 * 1.6e-19, velocity=vector(sqrt(2 * K0), 0, 0))
